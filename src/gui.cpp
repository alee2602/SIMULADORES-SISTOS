...
void drawTimelineSDL(const std::vector<ExecutionSlice>& timeline) {
    ...
    for (const auto& slice : timeline) {
        if (colorMap.find(slice.pid) == colorMap.end()) {
            colorMap[slice.pid] = getColor(colorIndex++);
        }
    }
    ...
    for (const auto& s : timeline)
        if (s.start + s.duration > maxTime)
            maxTime = s.start + s.duration;
    ...
    while (running) {
        ...
        for (const auto& s : timeline) {
            if (s.start > currentCycle) continue;
            int end = std::min(s.start + s.duration, currentCycle + 1);
            for (int t = s.start; t < end; ++t) {
                int x = startX + (t * (blockW + spacing)) - scrollX;
                SDL_Rect rect = {x, startY, blockW, blockH};
                SDL_Color c = colorMap[s.pid];
                SDL_SetRenderDrawColor(renderer, c.r, c.g, c.b, 255);
                SDL_RenderFillRect(renderer, &rect);
                renderText(renderer, font, s.pid, x + 8, startY + 20, {0, 0, 0, 255});
            }
        }
